...
export default function Gallery() {
  ...
  return (
    <div className="mx-auto md:px-8 lg:px-48 px-4 py-8 bg-indigo-100 pb-24">
      <h2 className="text-3xl font-bold mb-12 text-center mt-12"><Images className='inline mr-5'/><p className='inline'>Galería de Fotos</p> <Images className='inline ml-2' /></h2>
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {imagenes.slice(0, imagenesMostradas).map((imagen, index) => (
          <div key={index} className="relative aspect-video cursor-pointer overflow-hidden rounded-lg shadow-md transition-transform hover:scale-105">
            <Image
              src={imagen.src}
              alt={imagen.alt}
              layout="fill"
      ...
      {imagenesMostradas < imagenes.length && (
        <div className="mt-8 text-center">
          <Button className="bg-white" onClick={cargarMas} variant="outline">
            Mostrar más
          </Button>
        </div>
      )}
